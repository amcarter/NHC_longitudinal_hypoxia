...
NHC_sites <- read.csv("NC_synopticSamplingSites.csv", header=T, stringsAsFactors = F)

sites_sf <- NHC_sites[c(1:5,8:11),1:3] %>% 
  st_as_sf(coords=c("Long","Lat"),remove=F, crs=4326)

site_MCconf <- NHC_sites[14,1:3] %>%
  site_as_sf(coords=c("Long","Lat"), remove=F, crs=4326)
...
write.csv(siteDat, file = "NHC_sensorsite_data.csv", row.names=F)
