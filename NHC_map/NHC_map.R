...
sample_site_NHD_reaches <- tibble(comid=comid_points) %>%
  left_join(NHC_mainstem)%>% st_as_sf()
...
nhc_ws <- st_read("NHC_map/ws_shapefiles/nhc/wsboundary.shp",stringsAsFactors=FALSE)
mud_ws <- st_read("NHC_map/ws_shapefiles/mud/wsboundary.shp",stringsAsFactors=FALSE)
